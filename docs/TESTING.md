...
- **Suites:**
  ...
  - `time-options.test.ts` — the time-picker helpers (`lib/time-options.ts`): 15-minute option generation and 24h→12h label formatting.
...
- `golden-path.spec.ts` — the core tourist journey: an unonboarded tourist completes the tag quiz → lands on the personalised explore feed → opens a listing → schedules it into an itinerary → sees the success toast.
...
- `owner-theming.spec.ts` — the appearance switcher: on `/settings/profile`, selects a dark mode and the Sunset palette, then asserts both persist across a full navigation and a reload (the palette via the SSR cookie, the mode via next-themes). Runs as the **owner** — theming is role-agnostic, but `/settings/*` needs an onboarded account, and the tourist test user is reset to unonboarded (trapped in the quiz).
